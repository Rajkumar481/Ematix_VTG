...
const PatientDetailsDashboard = () => {
  ...
  const filteredDetails = useMemo(() => {
    return details.filter((detail) => {
      const matchesSearch =
        detail?.patient?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        detail?.medicineTakenPatient?.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      if (dateFilter !== "all") {
        ...
        switch (dateFilter) {
          ...
          case "custom":
            matchesDate =
              selectedDate &&
              visitDate.toDateString() === new Date(selectedDate).toDateString();
            break;
        }
      }
  ...
  }, [details, searchTerm, progressFilter, dateFilter, modeFilter, selectedDate]);
  ...
  return (
   <Box sx={{ backgroundColor: "#f5f5f5", minHeight: "100vh", p: { xs: 1, md: 3, }, mt: { xs: 4, md: 3, }}}>
